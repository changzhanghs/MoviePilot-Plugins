      ...
      <VWindowItem value="history">
        <section class="history-console">
          <div class="history-toolbar">
            ...
            <VSelect v-model="historyFilters.siteIds" :items="historySites" item-title="site_name" item-value="site_id" label="站点范围" multiple chips closable-chips variant="outlined" density="compact" hide-details />
          ...
          <template v-if="selectedHistoryPeriod">
            ...
            <section class="history-trend-panel history-overall-trend">
              ...
              <div v-if="historyScope === 'day' && !hourlyLoading && !hourlyTraffic.baseline_valid" class="history-chart-note">MP 在该日没有足够的相邻采样，暂无法计算小时增量</div>
            </section>

            <section class="history-site-panel">
              <div class="history-pane-heading"><div><strong>站点明细</strong><span>点击站点，在上方查看曲线，并在该行下方展开历史数据</span></div><VChip v-if="selectedHistorySite" size="small" color="primary" variant="tonal" closable @click:close="selectHistorySite(null)">{{ selectedHistorySite.site_name }}</VChip></div>
              <div class="history-detail-shell"><VTable fixed-header density="compact" class="history-detail-table"><thead><tr><th>站点</th><th>累计数据</th><th>周期增量</th><th>账号质量</th><th>做种</th></tr></thead><tbody><template v-for="item in selectedPeriodSites" :key="`${item.site_id || item.site_name}-${selectedHistoryPeriod.key}`"><tr class="history-site-row" :class="{ 'is-selected': selectedHistorySiteId && Number(item.site_id) === Number(selectedHistorySiteId) }" tabindex="0" @click="selectHistorySite(item)" @keydown.enter.prevent="selectHistorySite(item)"><td><div class="site-identity"><SiteAvatar :api="api" :site="item" :size="30" /><strong>{{ item.site_name }}</strong><VIcon :icon="Number(item.site_id) === Number(selectedHistorySiteId) ? 'mdi-chevron-up' : 'mdi-chevron-down'" size="17" /></div></td><td><span class="stacked-metric metric-upload">↑ {{ formatBytes(item.upload) }}</span><span class="stacked-metric metric-download">↓ {{ formatBytes(item.download) }}</span></td><td><span class="stacked-metric metric-upload">↑ {{ item.valid_count ? formatBytes(item.period_upload) : '基线不足' }}</span><span class="stacked-metric metric-download">↓ {{ item.valid_count ? formatBytes(item.period_download) : '基线不足' }}</span></td><td><span class="stacked-metric">分享率 {{ field(item.ratio, 3) }}</span><span class="stacked-metric">魔力 {{ bonusValue(item.bonus) }}</span></td><td><span class="stacked-metric">{{ formatNumber(item.seeding, 0) }} 个</span><span class="stacked-metric">{{ formatBytes(item.seeding_size) }}</span></td></tr><tr v-if="Number(item.site_id) === Number(selectedHistorySiteId)" class="history-site-expansion"><td colspan="5"><div class="history-expanded-heading"><div><strong>{{ item.site_name }} · 历史数据</strong><span>{{ selectedHistoryPeriod.startDay }}{{ historyScope === 'day' ? ' 00:00–23:59' : ` — ${selectedHistoryPeriod.endDay}` }}</span></div><VBtn size="small" variant="text" prepend-icon="mdi-close" @click.stop="selectHistorySite(null)">收起</VBtn></div><div class="history-record-shell"><VTable density="compact" class="history-record-table"><thead><tr><th>日期</th><th>累计上传</th><th>累计下载</th><th>当日上传</th><th>当日下载</th><th>分享率</th><th>积分 / 魔力</th><th>做种数 / 体积</th></tr></thead><tbody><tr v-for="record in selectedHistoryRecords" :key="`${record.site_id}-${record.updated_day}`"><td>{{ record.updated_day || '—' }}</td><td class="metric-upload">{{ formatBytes(record.upload) }}</td><td class="metric-download">{{ formatBytes(record.download) }}</td><td class="metric-upload">{{ record.baseline_valid ? formatBytes(record.daily_upload) : '基线不足' }}</td><td class="metric-download">{{ record.baseline_valid ? formatBytes(record.daily_download) : '基线不足' }}</td><td>{{ field(record.ratio, 3) }}</td><td>{{ bonusValue(record.bonus) }}</td><td><span class="stacked-metric">{{ formatNumber(record.seeding, 0) }} 个</span><span class="stacked-metric">{{ formatBytes(record.seeding_size) }}</span></td></tr></tbody></VTable></div></td></tr></template></tbody></VTable></div>
            </section>
          </template>
...
.history-detail-table th:nth-child(1){width:21%}.history-detail-table th:nth-child(2),.history-detail-table th:nth-child(3){width:22%}.history-detail-table th:nth-child(4){width:20%}.history-detail-table th:nth-child(5){width:15%}
.history-site-row{cursor:pointer;transition:background-color .15s ease}.history-site-row:hover,.history-site-row:focus{background:rgba(var(--v-theme-primary),.07);outline:none}.history-site-row.is-selected{background:rgba(var(--v-theme-primary),.16);box-shadow:inset 3px 0 rgb(var(--v-theme-primary))}
...
.history-site-expansion>td{padding:0!important;background:rgba(var(--v-theme-primary),.045)}.history-expanded-heading{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:12px 14px 7px}.history-expanded-heading>div{display:flex;min-width:0;flex-direction:column;gap:2px}.history-expanded-heading strong{font-size:.82rem}.history-expanded-heading span{color:rgba(var(--v-theme-on-surface),.52);font-size:.68rem}
.history-record-shell{overflow-x:auto;margin:0 12px 12px;border:1px solid var(--pt-border);border-radius:10px;background:rgba(var(--v-theme-surface),.42)}.history-record-table{min-width:920px;background:transparent}.history-record-table th{color:rgba(var(--v-theme-on-surface),.55)!important;font-size:.66rem;white-space:nowrap}.history-record-table td{font-size:.7rem;white-space:nowrap}
